        ...
        <div class="px-2">
            ...
            <v-data-table :headers="headers" :items="users" :loading="loading" class="elevation-1">
                ...
                <v-toolbar flat>
                  ...
                  <v-dialog v-model="dialogAddUser" max-width="600px">
                    <v-card>
                      ...
                      <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="blue darken-1" text @click="closeDialogAddUser">Cancel</v-btn>
              ...
              <template v-slot:[`item.actions`]="{ item }">
                <v-icon v-if="item.AlreadyInChat" title="User already at chat" style="background:green;" class="v-icon-action" color="white" large @click="showDialogAlreadyAdd()">
                  done_all
                </v-icon>
                <v-icon v-else style="background:orange;" class="v-icon-action" color="white" large @click="showDialogAddUser(item)">
                  send
                </v-icon>
              </template>
            </v-data-table>
  ...
  data() {
        return {
            ...
            headers: [
                ...
                { text: 'Add member', value: 'actions', sortable: false },
            ],
    ...
    methods: {
        ...
        async getUsers(search) {
          if (search) {
            let users;
            await axios.get(store.getters.URLS.API_URL + "chat/get_users_by_name?chatId=" + this.chatId + "&fullName=" + this.search)
            .then((response) => {
              users = response.data
            }).catch(() => {
