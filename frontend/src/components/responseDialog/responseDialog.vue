  ...
  <v-row justify="space-around">
    <v-col cols="auto">
      <v-dialog v-model="response.showResponseDialog" transition="dialog-bottom-transition" max-width="600">
        <v-card>
          <v-toolbar v-bind:color="response.success ? '#1976d2' : '#f00'" dark >
            {{ response.code }}
          </v-toolbar>
          <v-card-text>
            <div class="v-response-text text-h4 pa-12">
              {{ response.description }}
            </div>
          </v-card-text>
          <v-card-text>
            <div class="v-response-text text-h6 pa-12">
              {{ response.message }}
            </div>
          </v-card-text>
          <v-card-actions>
  ...
  data() {
    return {
      response: {
        ...
        code: "InternalError",
        description:
          "An error occurred on the server. Please try again later or contact your administrator",
        message: "",
      },
    };
  },
