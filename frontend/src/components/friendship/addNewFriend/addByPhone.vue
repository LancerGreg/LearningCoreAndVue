    ...
    <div class="px-2">
      ...
      <v-data-table :headers="headers" :items="users" :loading="loading" class="elevation-1">
          ...
          <v-toolbar flat>
            ...
            <v-dialog v-model="dialogInvite" max-width="600px">
              <v-card>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue darken-1" text @click="closeDialogInvite">
                    Cancel
                  </v-btn>
                  <v-btn color="blue darken-1" text @click="sendInvite">
                    Yes
                  </v-btn>
                  <v-spacer></v-spacer>
                </v-card-actions>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-icon
            v-if="!item.IsFriend && !item.HaveInvite"
            style="background: orange"
            class="v-icon-action"
            color="white"
            large
            @click="showInvite(item)"
          >
            send
          </v-icon>
          <v-icon
            v-if="!item.IsFriend && item.HaveInvite"
            title="the request is pending "
            style="background: blue"
            class="v-icon-action"
            color="white"
            large
            @click="showDialogWait()"
          >
            done
          </v-icon>
          <v-icon
            v-if="item.IsFriend"
            title="they are your friend"
            style="background: green"
            class="v-icon-action"
            color="white"
            large
            @click="showDialogFriendship()"
          >
            done_all
          </v-icon>
        </template>
  ...
  methods: {
        ...
        await axios
          .get(
            store.getters.URLS.API_URL +
              "friend/get_user_by_phone?userPhone=" +
              search
          )
          .then((response) => {
            users = response.data;
    ...
    sendInvite() {
      axios
        .post(
          store.getters.URLS.API_URL +
            "invite/invite_request_by_id?userId=" +
            this.users[this.editedIndex].UserId
        )
        .then(() => {
